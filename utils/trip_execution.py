...
def _process_order_manifest(trip_order, order_details, token):
    ...
    try:
        from api.biotrack import post_sublot, post_sublot_move, post_manifest
        
        # Create sublot
        sublot_result = post_sublot(order_details, token)
        if not sublot_result.get('success'):
            return {
                'order_id': trip_order.order_id,
                'status': 'failed',
                'error': f"Sublot creation failed: {sublot_result.get('error')}"
            }
        
        # Move sublot
        move_result = post_sublot_move(sublot_result['sublot_id'], trip_order.room_override, token)
        if not move_result.get('success'):
            return {
                'order_id': trip_order.order_id,
                'status': 'failed',
                'error': f"Sublot move failed: {move_result.get('error')}"
            }
        
        # Create manifest
        manifest_result = post_manifest(sublot_result['sublot_id'], trip_order.room_override, token)
        if not manifest_result.get('success'):
            return {
                'order_id': trip_order.order_id,
                'status': 'failed',
                'error': f"Manifest creation failed: {manifest_result.get('error')}"
            }
        
        # Update trip order with manifest ID
        trip_order.manifest_id = manifest_result['manifest_id']
        db.session.commit()
